...
@interface ControllerFillRepairInfo () <UITextFieldDelegate, UIPickerViewDelegate ,UIPickerViewDataSource>
{
    int                 _textFieldH;
...
@implementation ControllerFillRepairInfo
...
-(void)initUI
{
    ...
    //NSString* locationWds = @"[重新定位]";
    float locationWdsW = 0;//[Utils widthForString:locationWds fontSize:18] + 10;
    //UIButton* locationBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    //[locationBtn setFrame:CGRectMake(0, relationP.y, locationWdsW, height)];
    //[locationBtn setBackgroundColor:[UIColor clearColor]];
    //[locationBtn setTitle:locationWds forState:UIControlStateNormal];
    //[locationBtn setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    //locationBtn.titleLabel.font = [UIFont systemFontOfSize: 18.0];
    ...
    _longitudeLabel = [[UILabel alloc] initWithFrame:CGRectMake(locationWdsW, relationP.y, (frameR.size.width-locationWdsW)/2, height)];
    ...
    _latitudeLabel = [[UILabel alloc] initWithFrame:CGRectMake(locationWdsW+(frameR.size.width-locationWdsW)/2, relationP.y, (frameR.size.width-locationWdsW)/2, height)];
...
-(void)onConfirmClicked:(UITapGestureRecognizer*)sender
{
    ...
    {
        [MyAlertNotice showMessage:@"店铺信息不能为空" timer:2.0f];
        return;
    }
    ...
    //if ( _addrDic == nil )
    {
        [self requestAddRepairUser:addAddrDic];
    }
    //else
    {
...
-(void)requestAddRepairUser:(NSMutableDictionary*)addrDic
{
    // session
    ...
    {
        [MyAlertNotice showMessage:@"用户信息不存在，请重新登录！" timer:2.0f];
        return;
    }
    ...
    [net reqAddRepairUser:^(id success) {
